...
export const command = ["asahotak"];

global.asahOtak = global.asahOtak || {};
const ASAH_TIME_LIMIT = 60 * 1000;
...
export default async function asahOtakPlugin(msg, ctx) {
  ...
  try {
    ...
    if (replyTargetId && global.asahOtak[replyTargetId]) {
    ...
    console.error("AsahOtak Reply Error:", err);
  }

  if (ctx.command === "asahotak") {
    await reactm("⏳");

    try {
      const { data } = await axios.get("https://api.alpin-store.my.id/api/game/asahotak");
